...
class CamSubscriber(Node):
    def __init__(self):
        ...
        print("RECTANGLE-GATE Fucker is working")
    ...
    def image_sub_callback(self, msg):
        ...
        self.cnt += 1
        hahhaha=self.rectangle_det(cv2_img)

        #HasAA = color_detec(cv2_img)
    ...
    def rectangle_det(self,img):
        ...
        result = np.column_stack(np.where(gray!=0))
        result = result[:,::-1]
        #result is a numpy as train set

        if result.shape[0] > 3 :#at least 4 points in train set 


            #search centre
            km = KMeans(n_clusters=4)# we want to detect 4 clusters
            km.fit_transform(result)
            np.set_printoptions(threshold=np.inf)
            box = np.int0(km.cluster_centers_)
            #box is a n*2 numpy

            if box.shape[0] > 0:# at least, n is greater than 0, we can find center of the clusters_center
                for i in box:
                    cv2.drawMarker(img,i,(0, 0, 255),0,15,10)#

                hole_centre = np.int0(np.mean(box, 0)) 
                # hole_contre is a one dimensional numpy
            ...
            if box.shape[0] >= 2:## at least, n >=2 , we can use a line to calculate the distance
...
###################################################################################################################################
            ...
            if self.center_calibration1 == 1:
                ...
                # If it's not close enough, then fly to get close
                if self.distance_count < 4 and self.moveforward == 0:
                    time.sleep(1)
                ...
                if self.distance_count >= 4 and self.about_200cm == 0: 
